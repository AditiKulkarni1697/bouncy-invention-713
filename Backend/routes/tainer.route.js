...
trainerRouter.post("/otpverify", async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .send({ message: "somethin went wrong", error: error });
  }
});
...
trainerRouter.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (trainer.length == 0) {
      res
        .status(400)
        .send({ message: "Trainer not found, Kindly register", OK: false });
    } else {
      ...
      bcrypt.compare(password, Trainer.password, async (err, result) => {
        if (result) {
          ...
          res
            .status(401)
            .send({
              message: "Incorrect Password, Kindly Login Again",
              OK: false,
            });
        }
      });
...
trainerRouter.patch("/updateClass/:classID", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .send({ message: "class data updated", Class: classes });
  } catch (error) {
    return res
      .status(400)
      .send({ message: "Something went wrong", error: error.message });
