...
trainerRouter.post('/otpverify', async (req, res)=>{
    try {
        ...
        if(userOtp == correctOtp && correctOtp){
            bcrypt.hash(user.password,  process.env.saltRound, async function(err, hash) {
                if(err){
                    console.log(err)
                   return res.status(401).send({message:"Something went wrong",error:err, isOk : false });
                    ...
                    await trainer.save();
                    res.status(200).send({message:"Trainer Registered",trainer, isOk : !false})
                }
            }); 
        }
        else if(!correctOtp) return res.status(400).send({message : "Otp Expired! try again", isOk : false})
        else  return res.status(400).send({message : "invalid otp", isOk : false})
    } catch (error) {
        console.log(error)
        return res.status(400).send({message : "somethin went wrong", error : error, isOk : false})
    }
})
...
trainerRouter.post("/register", async (req,res)=>{
    ...
    try{
        ...
        if(trainer.length !== 0){
            return res.status(400).send({message:"trainer already registered", isOk: false})
        }else{
            let Email = getEmailForOtp(name)
            redisClient.setEx(email, 300, Email.otp +"")
            sendEmail(email,Email.otpSubject, Email.otpContent )
            return res.status(200).send({message : "otp send successfully to given email", isOk : true})
        }
    }catch(error){
        console.log(error)
...
trainerRouter.post('/login', async(req,res)=>{
    ...
    try {
        ...
        if(trainer.length == 0){
            res.status(400).send({message:"Trainer not found, Kindly register", "isOk" :  false})
        }else{
            ...
            bcrypt.compare(password, Trainer.password, async (err, result)=> {
                if(result){
                    ...
                    return res.status(200).send({message:"Login successful",token :tokenObj.token, refresh_token : tokenObj.refresh_token, "isOk": true, trainer:Trainer})
        ...
        res.status(400).send({message : 'someting went wrong', error : error, isOk:false})
    }
})
...
trainerRouter.get('/:trainerID', async (req, res)=>{
    try {
        ...
        if(trainer) return res.status(200).send({message : 'Trainer details feteched successfully.', Trainer : trainer, isOk: true})
